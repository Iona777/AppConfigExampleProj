...
namespace Selenium.UnitTestProject
{
    ...
    class SendApprovedLicenceViaEchosign
    {         
        ...
        public void SendApprovedLicenceViaEchosignTest()
        {
            using (IWebDriver driver = WebDriverManager.WebDriverManager.GetWebDriverForConfig())
            {
                ...
                AlterSkillsLicencePage.quoteATextBox(driver).SendKeys("7777.77");
                ...
                CustomerApprovalPage.getEchosignSection(driver).Click();
                string subHeadingText = CustomerApprovalPage.getTextForEchosignSubHeading(driver);
                Assert.IsTrue(subHeadingText.Contains("ADOBE"), subHeadingText + "doesn't contain 'ADOBE'");
                CustomerApprovalPage.selectQuoteAEchosign(driver).Click();
                CustomerApprovalPage.selectSendToCustomerViaEchosignButton(driver).Click();      
